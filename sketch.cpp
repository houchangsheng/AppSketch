...
DMatrix::DMatrix(int depth, int length, int width, int lgn, int K, int K2) {
...
val_tp DMatrix::Querynodeweight_d(uint32_t* key) {
    val_tp ret = 0;
    for (int j = 0; j < mv_.depth; j++) {
	unsigned long col = MurmurHash64A(key+1, mv_.lgn/2, mv_.hardner[j]) % mv_.length;
	val_tp value = 0;
	for (int i = 0; i < mv_.width; i++){
	    int index = j*mv_.width*mv_.length + i*mv_.length + col;
	    if (memcmp(key+1, mv_.counts[index]->key+1, mv_.lgn/2) == 0) {
		value += (mv_.counts[index]->sum + mv_.counts[index]->count)/2;
	    }
	    else value += (mv_.counts[index]->sum - mv_.counts[index]->count)/2;
	}
...
void DMatrix::QueryTopkApps(int K, std::pair<key_tp, val_tp> *es_topk_apps) {
    mymap nodeset;
    mymap edgeset;
    for (int k = 0; k < mv_.depth; k++) {
		...
		key_tp nodekey;
		nodekey.key[0] = mv_.counts[index]->key[0];
		if(nodeset.find(nodekey) == nodeset.end()) {
		    nodeset[nodekey] = Querynodeweight((uint32_t*)nodekey.key);
		}
            }
        }
    }
    ...
    for (int i = 0; i < K; i++) {
	es_topk_apps[i].second = 0;
    }

    for (auto it = nodeset.begin(); it != nodeset.end(); it++) {
	int i = K-1;
	if (it->second > es_topk_apps[i].second) {
	    while (it->second > es_topk_apps[i-1].second) {
		es_topk_apps[i] = es_topk_apps[i-1];
		i--;
		if (i == 0) {
		    break;
		}
	    ...
	    temp = std::make_pair(it->first, it->second);
	    es_topk_apps[i] = temp;
	}
    }
}

void DMatrix::QueryTopkUsers(int K2, std::pair<key_tp, val_tp> *es_topk_users) {
    mymap nodeset;
    mymap edgeset;
    for (int k = 0; k < mv_.depth; k++) {
        for (int i = 0; i < mv_.width; i++){
		...
		int index = k*mv_.width*mv_.length + i*mv_.length + j;
		key_tp nodekey;
		nodekey.key[1] = mv_.counts[index]->key[1];
		if(nodeset.find(nodekey) == nodeset.end()) {
		    nodeset[nodekey] = Querynodeweight_d((uint32_t*)nodekey.key);
		}
            }
        }
    }
    ...
    for (int i = 0; i < K2; i++) {
	es_topk_users[i].second = 0;
    }

    for (auto it = nodeset.begin(); it != nodeset.end(); it++) {
	int i = K2-1;
	if (it->second > es_topk_users[i].second) {
	    while (it->second > es_topk_users[i-1].second) {
		es_topk_users[i] = es_topk_users[i-1];
		i--;
		if (i == 0) {
		    break;
		}
