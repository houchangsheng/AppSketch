...
public:
    ...
    inline void New_Data(const DATA_TYPE& data, const COUNT_TYPE& p_s){
        DataNode* pData = new DataNode(data);
        Add_Count(min, pData, p_s);
        mp->Insert(data, pData);
    }

    inline void Add_Min(const COUNT_TYPE& p_s){
        Add_Data(((CountNode*)min->next)->pData->ID, p_s);
    }

    void Add_Data(const DATA_TYPE& data, const COUNT_TYPE& p_s){
    ...
    void Add_Count(CountNode* pCount, DataNode* pData, const COUNT_TYPE& p_s){
        COUNT_TYPE tempID = pCount->ID;
        if(!pCount->next)
            pCount->Connect(pCount, new CountNode(tempID + p_s));
        else {
            while((pCount->next)&&(pCount->next->ID - tempID < p_s)){
                pCount = (CountNode*)pCount->next;
            }
            if(!pCount->next)
                pCount->Connect(pCount, new CountNode(tempID + p_s));
            else {
                if(pCount->next->ID - tempID > p_s){
                    CountNode* add = new CountNode(tempID + p_s);
    ...
    void SS_Replace(const DATA_TYPE& data, const COUNT_TYPE& p_s){
